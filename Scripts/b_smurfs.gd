...
#func activate(e: InputEvent) -> void:
	#if Storage.OPENED_LOBBY.isCheckSmurfs == 0 and e is InputEventMouseButton and e.pressed:
		#button_pressed = true
		#disabled = true
		#if Storage.OPENED_LOBBY:
			#Storage.OPENED_LOBBY.isCheckSmurfs = 1
...
#
	#if Storage.OPENED_LOBBY:
		#var status = Storage.OPENED_LOBBY.isCheckSmurfs
		#if status == 1:
